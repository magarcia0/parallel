...
void verify(double a[DIM*DIM], double local_a[], double local_rhs[], double *b, double *x, double local_m, int my_rank, MPI_Comm comm, int start, int end);
...
void verify(double a[DIM*DIM], double local_a[], double local_rhs[], double *b, double *x, double local_m, int my_rank, MPI_Comm comm, int start, int end){
  ...
  for (local_i=start; local_i < end; local_i++)
  {
    ...
    for (j=start; j < end; j++)
    {
      ...
      if(check % 5 == 0 || start %5 == 0){
      //if(idx == DIM){a
        printf("RANK: %d, endmod5: %d\n",my_rank, (end-1)%5);
        idx++;
      }
      //local_rhs[idx] += a[j] * x[j];
